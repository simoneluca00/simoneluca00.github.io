...
<script>
    ...
    export default {
        ...
        mounted(){
            axios.get(`https://api.themoviedb.org/3/movie/${this.movieId}/credits?api_key=${this.propsApiKey}`)
            .then((resMovie) => {
                this.movieActorsArray = resMovie.data.cast;
            })
        ...
        computed: {
            ...
            movieActorsToPrint(){
               return this.movieActorsArray.map(element => {
        ...
        methods: {
            sliceMovieActors(){
                var fiveMovieActors = this.movieActorsToPrint.slice(0,5).join(' - ');
                return fiveMovieActors
            },
        }
